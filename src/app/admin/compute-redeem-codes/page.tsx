...
type ComputeRedeemCodeListResponse = {
  items: ComputeRedeemCodeItem[];
  total: number;
};

type ComputeRedemptionRecord = {
  id: number;
  code_mask?: string;
...
type ComputeRedemptionRecordResponse = {
  items: ComputeRedemptionRecord[];
  total: number;
};
...
export default function ComputeRedeemCodesPage() {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [data, setData] = useState<ComputeRedeemCodeListResponse>({ items: [], total: 0 });
  const [q, setQ] = useState("");
  const [status, setStatus] = useState("");
  const [batchNo, setBatchNo] = useState("");

  const [count, setCount] = useState(10);
  const [grantedPoints, setGrantedPoints] = useState(4950);
  ...
  const [records, setRecords] = useState<ComputeRedemptionRecordResponse>({ items: [], total: 0 });
  ...
  const loadCodes = useCallback(async () => {
    ...
    setError(null);
    try {
      const url = new URL("/api/admin/compute-redeem-codes", API_BASE);
      if (q.trim()) url.searchParams.set("q", q.trim());
      if (status) url.searchParams.set("status", status);
      if (batchNo.trim()) url.searchParams.set("batch_no", batchNo.trim());
      const res = await fetchWithAuth(url.toString());
      if (!res.ok) throw new Error(await res.text());
      const payload = (await res.json()) as ComputeRedeemCodeListResponse;
      setData(payload);
    } catch (err: unknown) {
  ...
  const loadRecords = async (codeID: number) => {
    ...
    setError(null);
    try {
      const res = await fetchWithAuth(`${API_BASE}/api/admin/compute-redeem-codes/${codeID}/redemptions`);
      if (!res.ok) throw new Error(await res.text());
      const payload = (await res.json()) as ComputeRedemptionRecordResponse;
      setRecords(payload);
  ...
  const createCodes = async () => {
    ...
    setGenerated(null);
    try {
      const res = await fetchWithAuth(`${API_BASE}/api/admin/compute-redeem-codes/generate`, {
        method: "POST",
  ...
  const updateCodeStatus = async (id: number, nextStatus: "active" | "disabled") => {
    setError(null);
    try {
      const res = await fetchWithAuth(`${API_BASE}/api/admin/compute-redeem-codes/${id}/status`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="space-y-6">
      <SectionHeader title="算力兑换码管理" description="生成算力点兑换码、管理状态、查看核销记录。" />
      ...
      <div className="rounded-3xl border border-slate-100 bg-white p-6">
        <h3 className="text-sm font-bold text-slate-700">批量生成算力兑换码</h3>
        <div className="mt-4 grid gap-4 md:grid-cols-4">
          ...
          <label className="space-y-1 text-xs text-slate-600">
            ...
            <input
              ...
              max={1000000000}
              value={grantedPoints}
              onChange={(e) => setGrantedPoints(Number(e.target.value || 1))}
              className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm"
            />
          </label>
          <label className="space-y-1 text-xs text-slate-600">
            <span>有效天数（0=不限制）</span>
            <input
              type="number"
              min={0}
              max={3650}
      ...
      <div className="rounded-3xl border border-slate-100 bg-white p-6">
        <div className="flex flex-wrap gap-3">
          ...
          <button onClick={loadCodes} className="rounded-lg border border-slate-300 px-4 py-2 text-xs text-slate-700 hover:bg-slate-50" disabled={loading}>
            {loading ? "加载中..." : "筛选"}
          </button>
        </div>
          ...
          <table className="w-full text-left text-xs text-slate-600">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th className="px-3 py-2">到账算力</th>
                <th className="px-3 py-2">有效天数</th>
                ...
                <tr key={item.id} className="border-t border-slate-100">
                  ...
                  <td className="px-3 py-2">
                    <div className="flex items-center gap-2">
                      <span className="font-medium text-slate-700">{item.code_plain?.trim() || item.code_mask}</span>
                      <button
                        type="button"
                  ...
                  <td className="px-3 py-2">{item.granted_points}</td>
                  ...
                  <td className="px-3 py-2">
                    <div className="flex flex-wrap gap-2">
                      <button onClick={() => loadRecords(item.id)} className="rounded border border-slate-200 px-2 py-1 hover:bg-slate-50">
                        核销记录
                      </button>
                      {item.status === "active" ? (
                        <button
              ...
              <tbody>
                {records.items.map((item) => (
                  <tr key={item.id} className="border-t border-slate-100">
                    ...
                    <td className="px-3 py-2">{item.granted_points || 0}</td>
                    <td className="px-3 py-2">{item.granted_starts_at ? new Date(item.granted_starts_at).toLocaleString() : "-"}</td>
                    <td className="px-3 py-2">{item.granted_expires_at ? new Date(item.granted_expires_at).toLocaleString() : "不限制"}</td>
